...
sub test {
    my (undef, $expect, $title, @detail) = @_;
    $number++;
    return unless $pass;
    chomp $expect;
    $title ||= $method;
...
# Test $number - $title
eod
    __PACKAGE__->echo (@detail) if @detail;
...
sub unload {
    $variable{result} = $su->_unload () if $pass;
    return;
}
